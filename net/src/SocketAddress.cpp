...
bool SocketAddress::init(const IPAddress& hostAddress, Poco::UInt16 portNumber) {
...
bool SocketAddress::init(const std::string& hostAddress, Poco::UInt16 portNumber) {
...
bool SocketAddress::init(Family fam, const std::string& address) {
#if defined(POCO_OS_FAMILY_UNIX)
	if (fam == UNIX_LOCAL) {
		//newLocal(address);
	...
	return init(fam, host, resolveService(port));
}


bool SocketAddress::init(const std::string& hostAndPort) {
	...
	return init(host, resolveService(port));
}
