...
#include "npoco/StringTokenizer.h"
...
{
#if !defined(POCO_OS_FAMILY_WINDOWS) && !defined(POCO_VXWORKS) && !POCO_OS_FAMILY_FREERTOS
	struct ifreq ifr;
...
inline const IPAddress& NetworkInterfaceImpl::address(unsigned index) const
{
	if (index < _addressList.size()) {
		return _addressList[index].get<NetworkInterface::IP_ADDRESS>();
	}
...
#elif POCO_OS == POCO_OS_LINUX || POCO_OS == POCO_OS_ANDROID || defined POCO_OS_FAMILY_FREERTOS
//
// Linux
//
...
#include <sys/types.h>
#if POCO_OS == POCO_OS_LINUX // Android API <23 & LwIP don't have <ifaddrs.h>.
#include <ifaddrs.h>
#elif POCO_OS == POCO_OS_ANDROID
#include <android/api-level.h>
#if __ANDROID_API__ > 23
#include <ifaddrs.h>	// Android API >23 has ifaddrs.h
#else
//#include <platforms/android/ifaddrs.h>
#endif
#endif
