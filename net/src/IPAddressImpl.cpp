...
#include <npoco/ByteOrder.h>
#include <npoco/NPString.h>
#include <npoco/Types.h>
...
{
	// Debug
	std::cout << "IPv4AddressImpl::parse() called." << std::endl;
	...
	// debug
	std::cout << "ia.s_addr: " << ia.s_addr << std::endl;
	
	if (ia.s_addr == INADDR_NONE && addr != "255.255.255.255") {
		// Debug
		std::cout << "Return Win32 IPv4AddressImpl()." << std::endl;
		
		return IPv4AddressImpl();
	}
	else {
		// Debug
		std::cout << "Return Win32 IPv4AddressImpl(&ia)." << std::endl;
	...
	if (ia.s_addr == INADDR_NONE && addr != "255.255.255.255") {
		// Debug
		std::cout << "Return GNUC < 3 IPv4AddressImpl()." << std::endl;
		return IPv4AddressImpl();
	}
	else {
		// Debug
		std::cout << "Return GNUC < 3 IPv4AddressImpl(&ia)." << std::endl;
		return IPv4AddressImpl(&ia);
	}
	...
	if (inet_aton(addr.c_str(), &ia)) {
		// debug
		// char *inet_ntoa(struct in_addr in); // back to IPv4 dotted notation.
		std::cout << "ia.s_addr: " << ia.s_addr << std::endl;
		std::cout << "Return IPv4AddressImpl(&ia)." << std::endl;
		
		return IPv4AddressImpl(&ia);
	}
	else {
		// Debug
		std::cout << "Return IPv4AddressImpl()." << std::endl;
