...
int SocketImpl::connect(const SocketAddress& address, const Poco::Timespan& timeout)
{
	if (_sockfd == POCO_INVALID_SOCKET)
	{
		init(address.af());
...
int SocketImpl::bind(const SocketAddress& address, bool reuseAddress) {
	return bind(address, reuseAddress, reuseAddress);
}
...
int SocketImpl::bind6(const SocketAddress& address, bool reuseAddress, bool ipV6Only) {
	return bind6(address, reuseAddress, reuseAddress, ipV6Only);
}


// Return values:
// 0	OK
// 1	SocketAddress isn't IPv6.
// 2	IPV6_V6ONLY not defined.
// 3	No IPv6 support available.
// 4	Bind failed.
int SocketImpl::bind6(const SocketAddress& address, bool reuseAddress, bool reusePort, bool ipV6Only) {
...
#else
	if (ipV6Only) {
		//throw Poco::NotImplementedException("IPV6_V6ONLY not defined.");
		return 2;
	}
#endif
	if (reuseAddress)
...
bool SocketImpl::checkBrokenTimeout(SelectMode mode) {
	if (_isBrokenTimeout) {
...
int SocketImpl::receiveFrom(void* buffer, int length, struct sockaddr** ppSA, 
					poco_socklen_t** ppSALen, int flags) {
	checkBrokenTimeout(SELECT_READ);
	int rc;
	do {
		if (_sockfd == POCO_INVALID_SOCKET) { //throw InvalidSocketException();
...
int SocketImpl::receiveFrom(SocketBufVec& buffers, struct sockaddr** pSA, 
										poco_socklen_t** ppSALen, int flags) {
	checkBrokenTimeout(SELECT_READ);
	int rc = 0;
	do {
...
bool SocketImpl::sendUrgent(unsigned char data) {
	if (_sockfd == POCO_INVALID_SOCKET) { //throw InvalidSocketException();
		return false;
	}
...
bool SocketImpl::setOption(int level, int option, const IPAddress& value) {
	return setRawOption(level, option, value.addr(), value.length());
}


bool SocketImpl::setOption(int level, int option, const Poco::Timespan& value) {
	struct timeval tv;
...
bool SocketImpl::setRawOption(int level, int option, const void* value, poco_socklen_t length) {
	if (_sockfd == POCO_INVALID_SOCKET) { //throw InvalidSocketException();
		return false;
	}
...
bool SocketImpl::getRawOption(int level, int option, void* value, poco_socklen_t& length) {
	if (_sockfd == POCO_INVALID_SOCKET) { //throw InvalidSocketException();
		return false;
	}
...
bool SocketImpl::setReusePort(bool flag) {
#ifdef SO_REUSEPORT
	//try
	//{
		int value = flag ? 1 : 0;
		return setOption(SOL_SOCKET, SO_REUSEPORT, value);
	/*}
	catch (IOException&)
	{
...
bool SocketImpl::init(int af) {
	return initSocket(af, SOCK_STREAM);
}


bool SocketImpl::initSocket(int af, int type, int proto) {
...
int SocketImpl::ioctl(poco_ioctl_request_t request, int& arg) {
#if defined(_WIN32)
...
int SocketImpl::ioctl(poco_ioctl_request_t request, void* arg)
{
#if defined(_WIN32)
	int rc = ioctlsocket(_sockfd, request, reinterpret_cast<u_long*>(arg));
#elif defined(POCO_VXWORKS)
