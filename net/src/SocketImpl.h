/*
	SocketImpl.h - Definition of the SocketImpl class.
*/
...
public:
	...
	virtual int connect(const SocketAddress& address, const Poco::Timespan& timeout);
	...
	virtual int bind(const SocketAddress& address, bool reuseAddress = false);
	...
	virtual int bind(const SocketAddress& address, bool reuseAddress, bool reusePort );
		/// Bind a local address to the socket.
		///
		/// This is usually only done when establishing a server
		/// socket. TCP clients should not bind a socket to a
	...
	virtual int bind6(const SocketAddress& address, bool reuseAddress = false, bool ipV6Only = false);
	...
	virtual int bind6(const SocketAddress& address, bool reuseAddress, bool reusePort,  bool ipV6Only);
		/// Bind a local IPv6 address to the socket.
		///
		/// This is usually only done when establishing a server
		/// socket. TCP clients should not bind a socket to a
	...
	virtual bool sendUrgent(unsigned char data);
	...
	bool setOption(int level, int option, const Poco::Timespan& value);
		/// Sets the socket option specified by level and option
		/// to the given time value.

	bool setOption(int level, int option, const IPAddress& value);
		/// Sets the socket option specified by level and option
		/// to the given time value.

	virtual bool setRawOption(int level, int option, const void* value, poco_socklen_t length);
		/// Sets the socket option specified by level and option
		/// to the given time value.
...
protected:
	...
	bool checkBrokenTimeout(SelectMode mode);
