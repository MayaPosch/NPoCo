...
#include "npoco/Environment.h"
#include <npoco/NumberFormatter.h>
#include <npoco/RWLock.h>
#include <npoco/TextIterator.h>
#include <npoco/TextConverter.h>
#include <npoco/UTF8Encoding.h>
#include <npoco/UTF32Encoding.h>
#include <npoco/Unicode.h>
#include <cstring>
...
std::string DNS::encodeIDNLabel(const std::string& label) {
	...
	while (it != end) {
		int ch = *it;
		if (ch < 0)  {
			//throw DNSException("Invalid UTF-8 character in IDN label", label);
			// TODO: Handle error.
			return std::string();
		}
...
std::string DNS::decodeIDNLabel(const std::string& encodedIDN)
{
	...
	if (encodedIDN.compare(0, 4, "xn--") == 0)
	{
		...
		if (rc == punycode_success)
		{
			...
			//throw DNSException("Failed to decode IDN label: ", encodedIDN);
			// TODO: Handle error.
			return std::string();
		}
	}
