/*
	Buffer.h - Definition of the Buffer class.
*/


#ifndef Core_Buffer_INCLUDED
#define Core_Buffer_INCLUDED
...
public:
	...
	/// is thrown.
	bool resize(std::size_t newCapacity, bool preserveContent = true) {
		if (!_ownMem) { return false; }
	...
	/// resized. If resize is attempted on those, IllegalAccessException
	/// is thrown.
	bool setCapacity(std::size_t newCapacity, bool preserveContent = true) {
		if (!_ownMem) { return false; }
		//throw Poco::InvalidAccessException("Cannot resize buffer which does not own its storage.");
