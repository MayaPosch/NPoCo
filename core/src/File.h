/*
	File.h - Definition of the File class.
*/


#ifndef Core_File_INCLUDED
#define Core_File_INCLUDED
...
public:
	...
	bool copyTo(const std::string& path, int options = 0) const;
	...
	bool moveTo(const std::string& path, int options = 0);
		/// Copies the file (or directory) to the given path and
		/// removes the original file. The target path can be a directory.
		/// If options is set to OPT_FAIL_ON_OVERWRITE the Method throws an FileExists Exception
		/// if the File already exists.

	bool renameTo(const std::string& path, int options = 0);
		/// Renames the file to the new name.
		/// If options is set to OPT_FAIL_ON_OVERWRITE the Method throws an FileExists Exception
		/// if the File already exists.

	bool linkTo(const std::string& path, LinkType type = LINK_SYMBOLIC) const;
...
protected:
	bool copyDirectory(const std::string& path, int options = 0) const;
