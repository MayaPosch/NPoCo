/*
	Mutex.h - Definition of the Mutex and FastMutex classes.
*/


#ifndef Core_Mutex_INCLUDED
#define Core_Mutex_INCLUDED
...
#include "platforms/Mutex_WINCE.h"
#else
#include "platforms/Mutex_WIN32.h"
#endif
#elif defined(POCO_VXWORKS)
#include "platforms/Mutex_VX.h"
#else
#include "platforms/Mutex_POSIX.h"
#endif
...
public:
	...
	bool lock(long milliseconds);
		/// Locks the mutex. Blocks up to the given number of milliseconds
		/// if the mutex is held by another thread, returns false.
		/// if the mutex can not be locked within the given timeout.
...
inline bool FastMutex::lock() {
	return lockImpl();
}


inline bool FastMutex::lock(long milliseconds) {
	if (!tryLockImpl(milliseconds)) { return false; }
	return true;
}
...
inline bool SpinlockMutex::lock(long milliseconds) {
	Timestamp now;
